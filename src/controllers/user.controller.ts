...
export const registerUser = async (req: Request, res: Response) => {
  ...
  let ARSAccount = await createAccount({money:1000})
  let USDAccount =  await createAccount({money:100})
...
export const getMe = async (req: Request, res: Response, next: NextFunction) => {
  ...
  const user = await (await (await findById(res.locals.payload._id,'-password -_id -__v -createdAt')).populate('USDAccount','-_id -__v -createdAt')).populate('ARSAccount','-_id -__v -createdAt')
  if (!user) return next(newError(404, "User not found"));
